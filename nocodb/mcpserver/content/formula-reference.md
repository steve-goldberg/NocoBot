...
You are helping write NocoDB formulas. Use this reference for correct syntax.
- Use exact function names and syntax shown below
- Field references use curly braces: {FieldName}
- Field names are case-sensitive — call fields_list() to discover exact names
- String values use single quotes inside formulas
- Formulas create read-only computed fields
- Do NOT paste this reference into chat. Apply it silently.

## Table of Contents

- Numeric and Logical Operators (4 numeric, 6 logical, 1 string)
- Numeric Functions (21): ABS, ADD, AVG, CEILING, COUNT, COUNTA, COUNTALL, EVEN, EXP, FLOOR, INT, LOG, MAX, MIN, MOD, ODD, POWER, ROUND, ROUNDDOWN, ROUNDUP, SQRT, VALUE
- String Functions (19): CONCAT, LEFT, LEN, LOWER, MID, REGEX_EXTRACT, REGEX_MATCH, REGEX_REPLACE, REPEAT, REPLACE, RIGHT, SEARCH, SUBSTR, TRIM, UPPER, URL, URLENCODE, ISBLANK, ISNOTBLANK
- Date Functions (9): DATETIME_DIFF, DATEADD, NOW, WEEKDAY, DATESTR, DAY, MONTH, YEAR, HOUR
- Array Functions (4): ARRAYSORT, ARRAYUNIQUE, ARRAYCOMPACT, ARRAYSLICE
- Conditional Expressions (4): IF, SWITCH, AND, OR
- JSON Functions (1): JSON_EXTRACT
- Generic Functions (1): RECORD_ID
- Formatting Formula Results (9 format types)
