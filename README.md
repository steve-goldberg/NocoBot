...
Under the hood, NocoBot connects an LLM (Claude, GPT, or any OpenRouter model) to NocoDB through the Model Context Protocol (MCP). The bundled MCP server exposes 62 tools covering the full NocoDB API, which the bot's agent loop calls as needed to fulfill your requests.
...
- **MCP Server** — 62 tools for Claude Desktop and AI integrations (FastMCP 3.x)
